...
| `clusterProvisioning` | `object` | Cluster provisioning configuration. See details below. |
...
#### `spec.clusterProvisioning`

| Field | Type | Description |
|---|---|---|
| `enabled` | `boolean` | If `true`, the operator will automatically provision the cluster via gRPC after pods are running. Defaults to `false`. |

> ⚠️ **Important**: When `clusterProvisioning.enabled` is set to `true`, you **must** disable all other forms of cluster provisioning:
> - Set `auto-provision = false` in `spec.config`
...
*   **`auto-provision`**: A boolean that controls whether the Restate node should automatically initialize itself. **When using `clusterProvisioning.enabled: true` (recommended), this must be set to `false`.** If not using operator-managed provisioning, this can be `true` for object-store metadata but must be `false` for `replicated` (Raft) metadata store.
