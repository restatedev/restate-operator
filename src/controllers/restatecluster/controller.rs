...
mod unit_tests {
    ...
    #[test]
    fn test_namespace_has_required_labels() {
        // Test helper function for verifying namespace labels
        let mut labels = BTreeMap::new();
        labels.insert("app.kubernetes.io/name".to_string(), "restate".to_string());
        labels.insert("app.kubernetes.io/instance".to_string(), "test-cluster".to_string());
        ...
        // This would be a helper function to verify namespace has required labels
        assert!(namespace.metadata.labels.is_some());
    ...
    #[test]
    fn test_restate_cluster_labels_generation() {
        // Test that RestateCluster has metadata labels (ResourceExt trait)
        let mut labels = BTreeMap::new();
        labels.insert("app.kubernetes.io/name".to_string(), "restate".to_string());
        labels.insert("app.kubernetes.io/instance".to_string(), "test-cluster".to_string());
        
        let cluster = RestateCluster {
            metadata: ObjectMeta {
            ...
            spec: RestateClusterSpec::default(),
            status: None,
        };

        let resource_labels = cluster.labels();
        assert_eq!(resource_labels, &labels);
        assert!(resource_labels.contains_key("app.kubernetes.io/name"));
        assert!(resource_labels.contains_key("app.kubernetes.io/instance"));
        assert_eq!(resource_labels.get("app.kubernetes.io/name").unwrap(), "restate");
        assert_eq!(resource_labels.get("app.kubernetes.io/instance").unwrap(), "test-cluster");
    }

    #[test]
    fn test_restate_cluster_annotations_generation() {
        // Test that RestateCluster has metadata annotations (ResourceExt trait)
        let mut annotations = BTreeMap::new();
        annotations.insert("restate.dev/managed-by".to_string(), "restate-operator".to_string());
        ...
        let resource_annotations = cluster.annotations();
        assert_eq!(resource_annotations, &annotations);
        assert!(resource_annotations.contains_key("restate.dev/managed-by"));
        assert_eq!(resource_annotations.get("restate.dev/managed-by").unwrap(), "restate-operator");
    }

    #[test]
    fn test_namespace_ownership_logic() {
        // Test our new namespace ownership logic
        let cluster_name = "test-cluster";
        let cluster_uid = "test-uid-123";
        
        // Create owner reference that represents our cluster
        let our_owner_ref = OwnerReference {
            api_version: "restate.dev/v1".to_string(),
            kind: "RestateCluster".to_string(),
        ...
        // Test case 1: Namespace with no owner references (we don't own it)
        let unowned_namespace = Namespace {
            metadata: ObjectMeta {
                name: Some("test-namespace".to_string()),
        ...
        let we_own_unowned = unowned_namespace
            .metadata
            .owner_references
            .as_ref()
            .map(|orefs| orefs.contains(&our_owner_ref))
            .unwrap_or(false);
        assert!(!we_own_unowned, "Should not own namespace with no owner references");
        
        // Test case 2: Namespace with different owner references (we don't own it)
        let other_owned_namespace = Namespace {
            metadata: ObjectMeta {
                name: Some("test-namespace".to_string()),
        ...
        let we_own_other_owned = other_owned_namespace
            .metadata
            .owner_references
            .as_ref()
            .map(|orefs| orefs.contains(&our_owner_ref))
            .unwrap_or(false);
        assert!(!we_own_other_owned, "Should not own namespace owned by someone else");
        
        // Test case 3: Namespace that we own
        let our_owned_namespace = Namespace {
            metadata: ObjectMeta {
                name: Some("test-namespace".to_string()),
                owner_references: Some(vec![our_owner_ref.clone()]),
                ..Default::default()
            },
            ..Default::default()
        };
        
        let we_own_our_owned = our_owned_namespace
            .metadata
            .owner_references
            .as_ref()
            .map(|orefs| orefs.contains(&our_owner_ref))
            .unwrap_or(false);
        assert!(we_own_our_owned, "Should own namespace that we created");
    }
}
