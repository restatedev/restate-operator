...
use crate::{
    Context, DocumentSpec, Metrics, RestateCluster, RestateClusterStatus, Result,
    DOCUMENT_FINALIZER,
};
...
impl RestateCluster {
    ...
    pub fn illegal() -> Self {
        let mut d = RestateCluster::new("illegal", DocumentSpec::default());
    ...
    pub fn test() -> Self {
        let mut d = RestateCluster::new("test", DocumentSpec::default());
        d.meta_mut().namespace = Some("default".into());
        d
    }
    ...
    pub fn with_status(mut self, status: RestateClusterStatus) -> Self {
...
impl ApiServerVerifier {
    ...
    async fn handle_finalizer_creation(mut self, doc: RestateCluster) -> Result<Self> {
    ...
    async fn handle_finalizer_removal(mut self, doc: RestateCluster) -> Result<Self> {
        let (request, send) = self.0.next_request().await.expect("service not called");
    ...
    async fn handle_status_patch(mut self, doc: RestateCluster) -> Result<Self> {
        let (request, send) = self.0.next_request().await.expect("service not called");
        assert_eq!(request.method(), http::Method::PATCH);
        assert_eq!(
            request.uri().to_string(),
        ...
        let status: RestateClusterStatus =
            serde_json::from_value(status_json).expect("valid status");
        assert_eq!(
